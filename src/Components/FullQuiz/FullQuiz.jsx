...
import { datafull } from "../../Assets/datafull";
...
function FullQuiz() {
  ...
  const [question, setQuestion] = useState(datafull[index]);
  ...
  const next = () => {
    if (lock === true) {
      if (index === datafull.length - 1) {
        setResult(true);
        return 0;
      }
      setIndex((i) => i + 1);
      setQuestion(datafull[index + 1]);
      setLock(false);
  ...
  return (
    <div className="container">
      ...
      <h1>Quiz Full-Stack</h1>
      <hr />
      {result ? (
        <>
          <img src={getImageForScore()} alt="" style={{ height: "400px" }} />

          <h2 className="result">
            You Scored {score} out of {datafull.length}{" "}
          </h2>
